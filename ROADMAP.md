...
2. **HTTP E2E** (`test:wechat-e2e`, etc.) and **desktop Electron** still have different depth. Desktop now has startup + main-flow + renderer-flow slices: `test:desktop-e2e`, `test:desktop-e2e:flow`, and `test:desktop-e2e:renderer` (startup assertions + deterministic main runtime chain + renderer-context preload/IPC orchestration + auto-exit). **Not yet**: full real-UI action automation (click/type against rendered controls) in one command.
...
2. **HTTP E2E**（`test:wechat-e2e` 等）与 **桌面 Electron** 仍有深度差异。现已提供启动 + 主进程 flow + 渲染器 flow 验收：`test:desktop-e2e`、`test:desktop-e2e:flow`、`test:desktop-e2e:renderer`（启动日志断言 + 主进程固定 runtime 事件链 + 渲染器上下文 preload/IPC 编排 + 自动退出）。尚未实现：一条命令完成真实 UI 控件动作（点击/输入）的完整桌面自动化。
