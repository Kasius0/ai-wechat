...
2. **HTTP E2E** (`test:wechat-e2e`, etc.) and **desktop Electron** still have different depth. A minimal desktop startup E2E command now exists: `test:desktop-e2e` (startup log assertions + auto-exit). **Not yet**: full desktop IPC/UI orchestration in one command.
...
2. **HTTP E2E**（`test:wechat-e2e` 等）与 **桌面 Electron** 仍有深度差异。现已提供最小桌面启动验收命令：`test:desktop-e2e`（启动日志断言 + 自动退出）。尚未实现：一条命令完成完整桌面 IPC/UI 编排断言。
