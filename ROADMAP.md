...
2. **Product slice (chosen: 2A)** — **SQLite persistence** (debounced + quit flush + **`wc-*` lifecycle purge**) + **`user_version` migration skeleton** + **sessionId** + **preload** are in place. **Next** for 2A: add **`migrationV1ToV2`** (etc.) when columns/tables change, and **encryption-at-rest** if required. **V2 trigger conditions:** (a) `runtime_sessions` schema shape changes, (b) data split into new tables, or (c) new indexes must exist for all installed DBs. **2B** desktop E2E stays **deferred**.
