...
2. **HTTP E2E** (`test:wechat-e2e`, etc.) and **desktop Electron** still have different depth. Desktop now has a minimum startup + runtime-flow slice: `test:desktop-e2e` and `test:desktop-e2e:flow` (startup assertions + deterministic runtime event chain in main process + auto-exit). **Not yet**: full desktop IPC/UI orchestration against real renderer actions in one command.
...
2. **HTTP E2E**（`test:wechat-e2e` 等）与 **桌面 Electron** 仍有深度差异。现已提供最小桌面启动 + runtime 流程验收：`test:desktop-e2e`、`test:desktop-e2e:flow`（启动日志断言 + 主进程固定 runtime 事件链 + 自动退出）。尚未实现：一条命令完成面向真实渲染器动作的完整桌面 IPC/UI 编排断言。
